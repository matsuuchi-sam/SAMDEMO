...
│   ├── generate/              ← e2studio 生成コード（スタートアップ・ベクタ・iodefine）
│   ├── linker/rx63n.ld        ← RX63N リンカスクリプト（ROM=0xFFF00000）
│   └── startup/               ← 旧スタートアップコード（現在未使用）
...
|------|-----|
| CPU | RX63N（RXv1 コア） |
| クロック | 電源投入時 LOCO 125kHz（HardwareSetup でメインクロック設定が必要） |
| フラッシュ | 2 MB（ROM: 0xFFE00000〜0xFFFFFFFF） |
| コードフラッシュ（rfp-cli認識） | 1 MB（0xFFF00000〜0xFFFFFFFF） |
| RAM | 128 KB（0x00000000〜0x0001FFFF） |
| 固定ベクタ | 0xFFFFFF80〜0xFFFFFFFF（リセットベクタ=0xFFFFFFFC） |
| UART0 TXD | P20（基板による） |
| UART0 RXD | P21（基板による） |
| LED | **Active High**（PODR=1 で点灯、PODR=0 で消灯）|
| LED ポート | 全ポート(P0〜PE)で点灯確認済み。個別ポート未特定 |

### GR-SAKURA 開発で判明した注意点

1. **LED は Active High**: CLAUDE.md 旧版の「Active Low」は誤り。PODR=1 で点灯する
2. **rfp-cli が認識するコードフラッシュは 0xFFF00000〜**: リンカスクリプトの .text を 0xFFE00000 から開始すると `[Warning] data outside the device's memory area` で書き込まれない。ROM 開始アドレスは **0xFFF00000** にすること
3. **スタートアップコード**: e2studio 生成の start.S は .data コピー・BSS クリア・ユーザーモード遷移を含む。最小限スタートアップ（mvtc USP/ISP + bsr _main のみ）でも動作する
4. **クロック**: HardwareSetup が空の場合、LOCO 125kHz で動作する。delay(10000) ≈ 1秒
5. **RUN モード切替時**: USB ケーブル挿し直しが必要な場合がある（SW1 だけでは不十分なことがある）
6. **GR-SAKURA は COM ポートではなく USB Direct 接続**: COMポート一覧には表示されない。接続確認は `rfp-cli -d RX63x -t usb -sig` で行うこと（COMポート検索は不要）
6. **GCC for RX のシンボル規約**: C シンボルに `_` プレフィックスが付く（例: C の `main` → アセンブラでは `_main`）
...
| Phase 1 | RX63N LED Lチカ | **完了** |
| Phase 2 | HOCO 50MHz + SCI0 UART | **完了** |
| Phase 3 | CMT0 タイマー (1ms 割り込み) | **完了** |
| Phase 4 | ソフト I2C + BME280 | **完了** |
| Phase 5 | JSON 送信 + コマンド受信 | **完了** |
| Phase 6 | PID 制御 + ヒーター GPIO | **完了** |
| Phase 7 | ESP32 BT ブリッジ + ダッシュボード | **完了** |

次のステップ: `make flash PORT=COMx` で書き込み → `tools/monitor.py` で動作確認。
